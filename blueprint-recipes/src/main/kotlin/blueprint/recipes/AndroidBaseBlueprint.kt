...
public class AndroidBaseBlueprint : Plugin<Project> {
  override fun apply(target: Project): Unit = with(target) {
    with(pluginManager) {
      apply("org.gradle.android.cache-fix")
    }

    val enableTestInDebug = boolPropertyOrElse(key = "blueprint.android.enableTestInDebug", default = true)
    val enableTestInRelease = boolPropertyOrElse(key = "blueprint.android.enableTestInRelease", default = false)

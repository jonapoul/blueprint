...
public class FileTree private constructor(private val mutableFiles: MutableMap<String, String>) {
  ...
  public operator fun set(path: String, contents: String): String? =
    mutableFiles.put(key = path, value = contents)
  ...
  public class Builder(private val relativeRootPath: String) {
    ...
    public operator fun String.invoke(config: Builder.() -> Unit) {
      ...
      files.putAll(Builder(subfolder).apply(config).build().files)
    }
